...
from .serializers import (
    RecipeSerializer, IngredientSerializer, AddFavorite,
    UserSerializer, FollowSerializer, AddAvatar
)
from recipes.models import (
    Recipe, Ingredient, Favorite, ShoppingCart,
    RecipeIngredient, RecipeShortLink, User, Follow
)
...
class FollowViewSet(UserViewSet):
    ...
    def subscriptions(self, request):
        ...
        if not queryset:
            return Response(ERRORS['no_subscriptions'],
                            status=status.HTTP_400_BAD_REQUEST)
    ...
    def perform_update(self, serializer):
        if serializer.instance.author != self.request.user:
            raise PermissionDenied(
                detail=ERRORS['cant_edit']
            )
        
    def perform_destroy(self, instance):
        if instance.author != self.request.user:
            raise PermissionDenied(
                detail=ERRORS['cant_delete']
            )
        instance.delete()
